...
class Navbar_Walker extends Walker_Nav_Menu{    
    ...
    if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
        ...
        // $class_names_filter = apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args );
        // if( isset( $class_names_filter )  && !empty( $class_names_filter ) )
            // $class_names = join( ' ', $class_names_filter );

        $class_names .= 'nav-item';
        if ( isset( $args->has_children ) && !empty( $args->has_children ) ) {
        ...
        // if ( in_array( 'menu-item', $classes, true ) && strpos($item->url,'#') === false ) {
        // 	$class_names .= ' active';
        // }
        ...
        if ( isset( $args->has_children ) && !empty( $args->has_children ) && 0 === $depth ) {
            $atts['href']           = '#';
        ...
        $item_output .= '<meta itemprop="name" content="'.$atts['title'].'">';
        if (isset( $args->link_before ) && !empty( $args->link_before )) 
            $item_output .= $args->link_before;
        
        $item_output .= apply_filters( 'the_title', $item->title, $item->ID );

        if (isset( $args->link_after ) && !empty( $args->link_after )) 
            $item_output .= $args->link_after;

        if(( isset( $args->has_children ) && !empty( $args->has_children ) && 0 === $depth )){
            $item_output .= '<span class="caret">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 11"><polyline points="0,0 22,0 11,11"></polyline></svg></span></a>';
        }else{
            $item_output .= '</a>';
        }
